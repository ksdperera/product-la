...
var meta = {
    "names": ["Server Id", "Server startup time (s)", "Server startup time (m)","Server startup time (h)"],
    "types": ["ordinal", "linear"]
};


var configChart = {
    title: "Server startup time distribution",
    type: "bar",
    x: "Server Id",
    colorScale:["#f1c40f"],
    charts: [{ y: "Server startup time (s)"}],
    width: $('body').width(),
    height: $('body').height(),
    padding: { "top": 40, "left": 80, "bottom": 70, "right": 100 },
    tooltip: {"enabled":true, "color":"#e5f2ff", "type":"symbol", "content":["Server Id","Server startup time (s)","Server startup time (m)", "Server startup time (h)"], "label":true}
};
...
function fetch(ch) {
    ...
    var queryInfo = {
        tableName: "LOGANALYZER_SERVERSTARTTIME",
        searchParams: {
            query : '(*:*)',
    ...
    client.search(queryInfo, function (d) {
        ...
        var obj = $.parseJSON(temObj );
            ...
            for (var i = 0; i < obj.length; i++) {
                    dataM.push([obj[i].values.AgentId, Math.round(obj[i].values.AverageTimeForStartup/1000 * 100) / 100,Math.round(obj[i].values.AverageTimeForStartup/60000 * 100) / 100,Math.round(obj[i].values.AverageTimeForStartup/3600000 * 100) / 100]);
}
