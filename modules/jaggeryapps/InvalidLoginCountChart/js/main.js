...
var meta = {
    "names": ["Server Id", "Invalid login attempts"],
...
var configChart = {
    ...
    x: "Server Id",
    colorScale:["#1abc9c"],
    charts: [{ y: "Invalid login attempts"}],
    width: $('body').width(),
    height: $('body').height(),
    padding: { "top": 40, "left": 80, "bottom": 70, "right": 100 },
    tooltip: {"enabled":true, "color":"#e5f2ff", "type":"symbol", "content":["Server Id","Invalid login attempts"], "label":true}
};
...
function fetch(ch) {
    ...
    var queryInfo = {
        tableName: "LOGANALYZER_INVALIDLOGCOUNT",
        searchParams: {
            query : '(*:*)',
    ...
    client.search(queryInfo, function (d) {
        ...
        var obj = $.parseJSON(temObj );
            ...
            for (var i = 0; i < obj.length; i++) {
                    dataM.push([obj[i].values.AgentId, obj[i].values.InvalidLoginCount, obj[i].values.InvalidLoginCount/3]);
}
